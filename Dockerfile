...
RUN wget http://archive.apache.org/dist/spark/spark-1.6.1/spark-1.6.1-bin-hadoop2.6.tgz -q \
...
RUN wget http://download.elastic.co/hadoop/elasticsearch-hadoop-2.2.0-rc1.zip -q \
    && mkdir /usr/local/spark/jars \
    && unzip elasticsearch-hadoop-2*.zip \ 
    && mv elasticsearch-hadoop-2.2.0-rc1/dist/elasticsearch-hadoop-2.2.0-rc1.jar /usr/local/spark/jars \
